...
export default async function EditProgram({ params }: { params: Promise<{ id: string }> }) {
  ...
  const action = isNew ? createProgram : updateProgram.bind(null, resolvedParams.id);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-800">
            {isNew ? "Tambah Program Spesial" : "Edit Program Spesial"}
          </h2>
          <p className="text-gray-600 text-sm mt-1">Atur program layanan khusus.</p>
        </div>
      </div>

      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden p-6 sm:p-8">
        <form action={action} className="space-y-8">
          <ProgramFormClient initialData={program} />
